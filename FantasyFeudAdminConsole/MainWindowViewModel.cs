...
public class MainWindowViewModel(IDialogCoordinator dialogCoordinator, IDataProcessor dataProcessor, IWebProcessor webProcessor) : BindableBase
{
    ...
    private List<QuestionsDataModel> _questionDataList = [];
    ...
    private ObservableCollection<TeamMembersDataModel> _team1Members = [];
    ...
    private ObservableCollection<TeamMembersDataModel> _team2Members = [];
    ...
    private List<AnswersDataModel> _answersData = [];
    ...
    private async void AddStrike()
    {
        ...
        _ = await dataProcessor.AddStrikeAsync(QuestionData);
    }
    ...
    private async void NextQuestion()
    {
        ...
        if (question is not null)
        {
            ...
            AnswersData = new(await dataProcessor.GetAnswersDataAsync(question.Id));
            SetConsole();
        }
    ...
    {
        GameData = await dataProcessor.GetGameDataAsync(GameData.Id);
        QuestionDataList = new(await dataProcessor.GetQuestionsDataAsync(GameData.Id));
        QuestionData = QuestionDataList.FirstOrDefault(_ => _.Id == QuestionData.Id) ?? new();
        Team1Data = await dataProcessor.GetTeamDataAsync(GameData.Team1Id);
        Team2Data = await dataProcessor.GetTeamDataAsync(GameData.Team2Id);
        Team1Members = new(await dataProcessor.GetTeamMembersDataAsync(Team1Data.Id));
        Team2Members = new(await dataProcessor.GetTeamMembersDataAsync(Team2Data.Id));
        AnswersData = new(await dataProcessor.GetAnswersDataAsync(QuestionData.Id));
    ...
    private async void ProcessChanges()
    {
        ...
        var success = await dataProcessor.UpdateAllDataAsync(Team1Data, Team2Data, GameData, Team1Members, Team2Members, QuestionData, AnswersData);

        _ = success
            ? await dialogCoordinator.ShowMessageAsync(this, "Success!", "Data sync'd successfully.")
            : await dialogCoordinator.ShowMessageAsync(this, "Failure!", "Data NOT sync'd successfully.");
    }

    private async void AddTeamMember(int? teamNumber)
    {
        var teamName = teamNumber == 1 ? Team1Name : Team2Name;
        var name = await dialogCoordinator.ShowInputAsync(this, "Insert Name", $"Insert new player for {teamName}");
        ...
        _ = await dataProcessor.AddTeamMemberAsync(newMember);
    ...
    private async void RemoveTeamMember(int? teamNumber)
    {
        ...
        {
            id = SelectedTeam1Member.Id;
            _ = Team1Members.Remove(SelectedTeam1Member);
        }
        else if (teamNumber == 2 && SelectedTeam2Member is not null)
        {
            id = SelectedTeam2Member.Id;
            _ = Team2Members.Remove(SelectedTeam2Member);
        }

        _ = await dataProcessor.RemoveTeamMemberAsync(id);
    }

    private async void PostEvent()
    {
        List<TeamMembersDataModel> teamMembersData = [.. Team1Members, .. Team2Members];
        QuestionModel model = webProcessor.CreateEventMessage(GameData, new List<TeamsDataModel>() { Team1Data, Team2Data }, teamMembersData, QuestionData, AnswersData);
        Debug.WriteLine(await webProcessor.PostEventAsync(model));
    }
    ...
    private async void ChangeActiveMember(int? teamNumber)
    {
        ...
        if (teamNumber == 1 && SelectedTeam1Member is not null)
        {
            ...
            active.Active = 1;
            _ = await dataProcessor.ChangeActiveMemberAsync(inactive.Id, active.Id);
    ...
    private async void AwardTeam(int? teamNumber)
    {
        ...
        _ = await dataProcessor.AwardPointsAsync(GameData.Id, teamNumber ?? 0, teamNumber == 1 ? GameData.Team1Score : GameData.Team2Score);
    }
    ...
    private void SetConsole()
    {
        ...
        Team1Score = GameData.Team1Score;
        SelectedTeam1Member = Team1Members.FirstOrDefault(_ => _.Active == 1) ?? new();
    ...
    private async void ShowAnswer(int? index)
    {
        ...
        AnswersDataModel model = AnswersData[(int)index];
        ...
        _ = await dataProcessor.ShowAnswerAsync(model);
    }
